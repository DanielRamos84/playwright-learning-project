...
test('Checkout Process', async ({ page }) => {
    ...
    for (let i = 0; i < getProductsCount; i++) {
        const productMatchFound = await getProductsRows.nth(i).locator('h5').textContent();
        if (productMatchFound === testProduct) {
            await getProductsRows.nth(i).getByRole('button', { name: /add to cart/i }).click();
            break;
        }
    }

    await expect(page.getByRole('alert', { name: /product added to cart/i })).toBeVisible();
    ...
    const applyCouponLabelInput = applyCouponLabel.locator('..').locator('[type="text"]');
    ...
    await page.getByPlaceholder('Select Country').pressSequentially(country, { delay: 100 });
    ...
    const confirmationNumber = await page.locator('label.ng-star-inserted').textContent();
    ...
    for (let i = 0; i < rowsCount; i++) {
        ...
        if (confirmationNumber.includes(findOrderNumberText)) {
