...
test('Navigate Back and Forth From Parent to Child Window', async ({ browser }) => {
    ...
    const interviewQuestionsLink = page.getByRole('link', { name: "Free Access to InterviewQues/ResumeAssistance/Material" })
    ...
    console.log(`Navigating to new url` + await newPage.evaluate('location.href'));

    //grab email address found in child window so we can enter it in the parent window
    const storeEmailAddress = await newPage.locator('.im-para.red a').textContent();
